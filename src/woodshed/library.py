...
Folders are the source of truth. Each take carries its transcript in its own ID3 tags,
...
def _read(path: Path, song: str) -> Take:
    ...
    return Take(path, song, field(_TRANSCRIPT), int(genius_id) if genius_id.isdigit() else None)
...
class Library:
    ...
    def add_take(self, src: Path, song: str | None, start: float, end: float, recorded: datetime,
                 lines: list[str], genius_id: int | None = None, artist: str | None = None) -> Path:
