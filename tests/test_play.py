...
def songs(tmp_path, tone, monkeypatch):
    ...
    for day, metrics in ((14, Metrics(22, 0.01)), (2, Metrics(15, 0.02)), (30, Metrics(6, 0.05))):
...
def test_rating_plays_the_worst_take_then_the_best(songs):
    # overall = 0.6 pitch + 0.4 timing: the 2nd rates 6.2, the 14th 4.9, the 30th 6.5
    result = play(songs, "harbor lights", "--rating")
    assert result.exit_code == 0, result.output
    assert played() == ["14", "30"]
    assert "worst take by rating (2 of 3)" in text(result) and "rated 6.5/10" in text(result)
...
def test_progress_rates_older_takes_once(songs, monkeypatch):
    ...
    monkeypatch.setattr(rating, "analyze", lambda stems: Metrics(10, 0.01))
    monkeypatch.setattr(cli.models, "missing", lambda: [])
    runner = CliRunner()

    first = runner.invoke(cli.app, ["progress", "winter", "--library", str(songs.root)])
    assert first.exit_code == 0, first.output
    assert "Winter Town" in first.output and "8.5" in first.output and len(calls) == 1
...
def test_progress_shows_every_take_and_the_trend(songs):
    ...
    assert result.exit_code == 0, result.output
    assert "22¢ off" in text(result) and "±5.0%" in text(result)
...
def test_stricter_reference_points_lower_every_score(songs, tmp_path, monkeypatch):
    ...
    result = CliRunner().invoke(cli.app, ["progress", "harbor lights", "--library", str(songs.root)])
    assert result.exit_code == 0, result.output
    # the 30th: pitch 6¢ now scores 6.0 (was 9.5), so overall 0.6 * 6.0 + 0.4 * 2.0 = 4.4 (was 6.5)
    assert "6.0 6¢ off" in text(result) and "best 4.4 (take 3)" in text(result)
